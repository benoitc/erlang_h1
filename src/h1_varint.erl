...
%% variable-length encoding with 1, 2, 4, or 8 bytes. The two most
%% significant bits indicate the encoding length.
%%
...
incomplete_test_() ->
    [
        ...
        ?_assertEqual({error, incomplete}, decode(<<64>>)),
        ?_assertEqual({error, incomplete}, decode(<<128, 0, 0>>)),
        ?_assertEqual({error, incomplete}, decode(<<192, 0, 0, 0, 0, 0, 0>>))
    ].
